...
export default function NewBetModal({ roomId, onClose, onCreated }: Props) {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    if (betType !== 'binary' && validOptions.length < 2) { setError('Adicione pelo menos 2 opções.'); return }
    setLoading(true); setError('')
    try {
      const { data: { session } } = await supabase.auth.getSession()
      const { data: bet, error: betErr } = await supabase.from('bets')
        .insert({ room_id: roomId, title: title.trim(), bet_type: betType, status: 'open', created_by: session?.user?.id || null })
        .select().single()
      ...
      <div className="card w-full max-w-md animate-slide-up">
        <div className="p-5 border-b border-[#1E2D24] flex items-center justify-between">
          <div className="flex items-center gap-3">
            {step === 'details' && <button onClick={() => setStep('type')} className="text-[#4A6658] hover:text-white text-sm">← Voltar</button>}
        ...
        {step === 'type' && (
          <div className="p-5 space-y-3">
            ...
            {BET_TYPES.map(bt => (
              <button key={bt.type} onClick={() => { setBetType(bt.type); setStep('details') }}
          ...
          <form onSubmit={handleCreate} className="p-5 space-y-4">
            <div>
              ...
              <textarea className="input resize-none" rows={2}
                placeholder={betType === 'choice' ? 'Ex: Quem vai beber mais chopp?' : 'Ex: O cachorro vai fazer coco?'}
