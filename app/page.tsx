...
export default function Home() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    } catch { setError('Erro ao criar sala. Tente novamente.') }
  ...
  async function handleJoin(e: React.FormEvent) {
    ...
    } catch { setError('Sala não encontrada. Confira o código.') }
    finally { setLoading(false) }
  }
        ...
        <div className="p-6">
          {tab === 'create' ? (
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div>
                <label className="block text-xs text-[#4A6658] mb-1.5 font-medium uppercase tracking-wider">Seu nome (Admin)</label>
                <input className="input" placeholder="Como te chamam?" value={adminName} onChange={e => setAdminName(e.target.value)} required />
              </div>
              <div>
                <label className="block text-xs text-[#4A6658] mb-1.5 font-medium uppercase tracking-wider">Sua chave PIX <span className="font-normal normal-case">(opcional)</span></label>
            ...
            <form onSubmit={handleJoin} className="space-y-4">
              ...
              <p className="text-xs text-[#4A6658] text-center">Peça o código para quem criou a sala</p>
